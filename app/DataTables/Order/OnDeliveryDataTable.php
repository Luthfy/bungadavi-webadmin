...
class OnDeliveryDataTable extends DataTable
{
    /**
    ...
    public function query()
    {
        $model = new Order();
        if (auth()->user()->hasRole('bungadavi')) {
            return $model->orderBy('created_at', 'desc')->where('status_order_transaction', "On Delivery")->newQuery();
        } else {
            return $model->where('florist_uuid', auth()->user()->customer_uuid)->where('status_order_transaction', "On Delivery")->newQuery();
        }
    }
        ...
        return datatables()
            ...
            ->addColumn('action', function ($datatable) {
                ...
                if (auth()->user()->hasRole('bungadavi')) {
                    ...
                    $html .= "<a href='".route('bungadavi.orders.print_do', ['transaction' => $datatable->uuid])."' class='text-warning m-1' target='_blank'><span class='fa fa-print'></span> Print DO</a>";
                } else if(auth()->user()->hasRole('corporate')){
                    $html .= "<a href='".route('corporate.orders.show', ['transaction' => $datatable->uuid])."' class='text-primary m-1'><span class='fa fa-eye'></span></a>";
                } else {
                    $html .= "<a href='".route('affiliate.orders.show', ['transaction' => $datatable->uuid])."' class='text-primary m-1'><span class='fa fa-eye'></span></a>";
