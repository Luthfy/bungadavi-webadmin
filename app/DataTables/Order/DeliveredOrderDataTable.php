...
class DeliveredOrderDataTable extends DataTable
{
    ...
    public function query()
    {
        $model = new Order();
        if (auth()->user()->hasRole('bungadavi')) {
            return $model->orderBy('created_at', 'desc')->where('status_order_transaction', "Delivered")->newQuery();
        } else {
            return $model->where('florist_uuid', auth()->user()->customer_uuid)->where('status_order_transaction', "Delivered")->newQuery();
        }
    }
